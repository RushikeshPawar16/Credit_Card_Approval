...
class PredictModel:
    ...
    ****************************************************************************
    ...
    def batch_predict_from_model(self):
        ...
        *   none:
        ...
        try:
            ...
            # file_name = 'apps/models/RandomForest'
            # if file_name == 'apps/models/XGB':
            #     file_name = os.path.basename(file_name.path)
            # elif file_name == 'apps/models/RandomForest':
            #     file_name = os.path.basename(file_name.path)

            model = self.fileOperation.load_model('RandomForest')
            y_predicted = model.predict(self.X)
            result = pd.DataFrame({"Prediction": y_predicted})
    ...
    #         # cluster selection
    #         clusters = kmeans.predict(self.X.drop(['Zipcode'], axis=1))
    ...
    #             cluster_data_new = cluster_data.drop(['Zipcode', 'clusters'], axis=1)
